 ...
 *
 * This command is used for translating data source to diagram Doctrine entities in multiple DataSourceToDiagramMappings.
 */
class ProcessDataSourceToDiagramMappingsCommand extends ContainerAwareCommand
{
    /**
     * @inheritdoc
     */
    protected function configure()
    {
        $this
            ...
            ->setName('app:process-entity-mappings')

            // the short description shown while running "php app/console list"
            ->setDescription('Processes multiple DataSourceToDiagramMappings.')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('This command allows you to translate data source Doctrine entities into diagram Doctrine entities, based on multiple DataSourceToDiagramMapping configurations which share the same diagram.')
    ...
    {
        $lockHandler = new LockHandler('process_mappings.lock');
        if (!$lockHandler->lock()) {
        ...
        $dataSourceToDiagramMappings = $repository->findAllByDiagramName($diagramName);
